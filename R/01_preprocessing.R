# ============================================================================
# PREPROCESSING - CPTAC COLORECTAL CANCER PROTEOMICS
# ============================================================================
#
...
# Output: 3 clean CSV files + summary statistics
#
# Dataset: CPTAC TMT-labelled quantitative proteomics
#          Colorectal cancer - matched normal vs tumour tissue
# ============================================================================
...
# SETUP
# ============================================================================
# Set base_dir to the folder containing your raw CPTAC Excel files
base_dir <- "data/raw"
sample_file <- file.path(base_dir, "CPTAC_sample_information.xlsx")
data_file   <- file.path(base_dir, "CPTAC_data.xlsx")

output_dir <- "data/processed"
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

cat(paste(rep("=", 80), collapse = ""), "\n")
cat("PREPROCESSING - CPTAC DATA\n")
cat(paste(rep("=", 80), collapse = ""), "\n\n")
...
cat(sprintf("   Sample info table: %d rows x %d cols\n", nrow(sample_df), ncol(sample_df)))
...
# Select only the TMT columns present in both metadata and data
...
cat(sprintf("   Samples: %d (60 total - 6 internal standards)\n\n", n_samples))
...
write.csv(mapping_clean,
          file.path(output_dir, "CPTAC_sample_metadata.csv"),
          row.names = FALSE)
cat("   Saved: CPTAC_sample_metadata.csv\n")

write.csv(expr_clean,
          file.path(output_dir, "CPTAC_expression_matrix.csv"),
          row.names = FALSE)
cat("   Saved: CPTAC_expression_matrix.csv\n")

col_annot <- mapping_clean %>%
  select(Column_Name, SampleSet, TMT_Channel, Patient_ID, Tissue_Type)
write.csv(col_annot,
          file.path(output_dir, "CPTAC_column_annotation.csv"),
          row.names = FALSE)
cat("   Saved: CPTAC_column_annotation.csv\n\n")
